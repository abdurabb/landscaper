...
const PostCard = ({ product }) => {
    ...
    // content:
    const name = product?.name ? product?.name[lang] || product?.name["en"] : "";
    const postTime = product?.postTime ? product?.postTime[lang] || product?.postTime["en"] : "";
    const postTitle = product?.postTitle ? product?.postTitle[lang] || product?.postTitle["en"] : ""
    const image = product?.images[0]
    const commentsCount = product?.commentsCount

    return (
        <div
            dir={isArabic ? "rtl" : "ltr"}
            className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden max-w-md mx-auto hover:shadow-md transition-shadow duration-200">
            <div className="flex">
                <div className="relative w-24 flex-shrink-0">
                    <img
                        src={`${image || 'https://images.unsplash.com/photo-1585320806297-9794b3e4eeae?w=400&h=250&fit=crop'}`}
